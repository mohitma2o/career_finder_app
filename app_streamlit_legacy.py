    ...
    with col1:
        ...
        with btn2:
            if st.button("Explore Database", use_container_width=True):
                ...
                st.rerun()
        with btn3:
            # File uploader to load profile JSON (full width for symmetry)
            uploaded_file = st.file_uploader("Load Profile JSON", type=["json"], label_visibility="collapsed", help="Upload a previously saved JSON profile.")
